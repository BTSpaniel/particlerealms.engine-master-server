...
from .hub import Hub
from .protocol import PROTOCOL_VERSIONS, ProtocolError, make_error, validate_envelope
        ...
        try:
            while True:
                ...
                if not sess["proven"] and msg_type not in ("HELLO", "PROVE", "PING"):
                    ...
                    await websocket.send_json({"protocol": PROTOCOL_VERSIONS["SESSION"], "type": "PONG"})
                ...
                if not sess["proven"] and time.monotonic() > proof_deadline:
...
async def _send_error(websocket: WebSocket, code: str, message: str) -> None:
    try:
        await websocket.send_json(make_error(code, message))
    except Exception:
        pass
...
async def _handle_prove(hub: Hub, session_id: str, sess: dict, websocket: WebSocket, data: dict) -> None:
    ...
    sess["proven"] = True
    hub.touch_heartbeat(session_id)
    await websocket.send_json({"protocol": PROTOCOL_VERSIONS["SESSION"], "type": "PROVE", "payload": {"ok": True}})
...
async def _handle_attach_route(hub: Hub, config: Config, session_id: str, websocket: WebSocket, data: dict) -> None:
    ...
    if not route_id:
        ...
        return
    try:
        hub.attach_route(session_id, route_id)
    except ValueError:
        await _send_error(websocket, "too-many-routes", f"session already attached to {config.max_routes_per_session} routes")
        return
    await websocket.send_json({
...
async def _handle_route_message(hub: Hub, config: Config, session_id: str, websocket: WebSocket, data: dict, msg_type: str) -> None:
    ...
    for sid in subscribers:
        target = hub.sessions.get(sid)
        if target is None:
            continue
        try:
            await target["ws"].send_json(outgoing)
        except Exception:
            pass
